...
const newWords = JSON.parse(fs.readFileSync('c:\\Users\\FP123\\Downloads\\KBVLYON\\words_to_add_v5.json', 'utf8'));
const improvements = JSON.parse(fs.readFileSync('c:\\Users\\FP123\\Downloads\\KBVLYON\\improvements_v5.json', 'utf8'));

console.log(`📝 TRAITEMENT DE LA VAGUE 5`);
console.log(`   - Nouveaux mots: ${newWords.length}`);
console.log(`   - Améliorations: ${improvements.length}`);

// 1. Appliquer les améliorations de traduction
console.log('\n🔧 Application des améliorations de traduction...');
improvements.forEach(imp => {
  const entryIndex = currentDictionary.findIndex(e => e.id === imp.id);
  if (entryIndex !== -1) {
    // On garde l'ancienne traduction en note si elle est très différente ?
    // Non, on remplace pour l'instant, c'est plus propre.
    console.log(`   ✨ ${imp.word}: "${imp.currentTranslation}" -> "${imp.newTranslation}"`);
    currentDictionary[entryIndex].translation.pt = imp.newTranslation;
    
    // Mettre à jour la catégorie si elle était "Geral" ou manquante
    if ((!currentDictionary[entryIndex].category || currentDictionary[entryIndex].category === 'Geral') && imp.category) {
        currentDictionary[entryIndex].category = imp.category;
    }
  }
});
...
// Templates d'exemples (réutilisés)
const templates = {
  'Verbe': {
    pt: (word, translation) => {
  ...
  'Conjonction': {
    pt: (word, translation) => [`Use "${translation}" na frase`],
    kea: (word) => [`Uza "${word}" na frazi`]
  },
  'Préposition': {
    pt: (word, translation) => [`Vou ${translation} casa`],
    kea: (word) => [`N ta bai ${word} kasa`]
  }
};
...
console.log('\n✅ Dictionnaire mis à jour avec succès (V5)!');
